...
public class TerrainTileFX extends StackPane {
    ...
    public void updateTileView() {
        if (tile.isEmpty()) {
            ...
            getChildren().setAll(background, overlay);
        } else {
            overlay.setFill(tile.getOccupant().getColor());
            Image image = tile.getOccupant().getImage();
            ImageView icon2 = new ImageView(image);
            getChildren().setAll(background, overlay, icon2);
        }
        if (this == GameController.getLastClicked()) {
            overlay.setFill(Color.rgb(255, 255, 255, 0.2));
        }
    }
}
